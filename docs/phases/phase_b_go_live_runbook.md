...
| Entity-aware breaker (curative) | `5cf0706` | `EVENT_BREAKER_ENABLED` (OFF) |
...
- **4c. Gate + breaker.** Set `EVENT_ENTITY_MIN > 0` and `EVENT_BREAKER_ENABLED`. The matcher now uses entities; the breaker splits the existing blob. Let the graph **settle** over several cycles. **GATE:** blob dissolves, new matching coherent, dossier ids stable (keep-core preserves them), no oscillation. *(Backup taken before this step.)*

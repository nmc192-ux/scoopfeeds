...
Phase 1 — Word timings. Switch narration to ElevenLabs' with-timestamps endpoint so every word has a start and end time. Build word-by-word captions from them. VOICE DECIDED by DrJ 23 Sep 2026: his cloned "Dr J" voice CD9jldcGVbcDmJckCEwY on eleven_multilingual_v2, replacing Daniel. Make it env-only (VIDEO_VOICE_ID, VIDEO_VOICE_MODEL) so reverting is one line plus a recreate. Land it in the same deploy as this phase so the TTS cache refills once. Note multilingual_v2 bills 1 credit per character against turbo's 0.5. After the flip, confirm a short actually publishes (the heartbeat stays green on zero output).
VOICE REVISED by DrJ 24 Sep 2026 (ElevenLabs credit constraint): KEEP DANIEL on eleven_turbo_v2 for now. Phase 1 shipped the word-by-word captions only (VIDEO_WORD_CAPTIONS_ENABLED=1); VIDEO_VOICE_ID / VIDEO_VOICE_MODEL are NOT changed in prod. The Dr J voice is a two-line env switch for later (VIDEO_VOICE_ID=CD9jldcGVbcDmJckCEwY, VIDEO_VOICE_MODEL=eleven_multilingual_v2, then recreate); the offline Phase 1 sample proved it works end to end with the captions. Acceptance: one offline short whose captions land on the words.
...
Photo rules (DrJ, 24 Sep 2026 — added after the Phase 1 sample showed the article's own photo, an Instagram screenshot of the accused, on a crime story):
- Never use the publisher's own article photo (article.image_url or any rendition of it) as shot imagery.
- Never use a private individual's personal photo or a social-media screenshot, especially in crime stories. Public figures in official or licensed imagery are fine; a private person's own posts are not, whoever is credited.
Hard-won facts:

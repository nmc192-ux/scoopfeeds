...
**2026-07-04 (Session, cont.) — BLUESKY FIX DEPLOYED + KERNEL REBOOT DONE.** *(M6 still has one item left — Caddy / strict-TLS.)*
- **BLUESKY:** Diagnosed the social-posting gap — Facebook/Instagram/X posting fine, but Bluesky fully jammed (**749** "createSession on cooldown" log lines, **zero** posts) + one transient Threads 500. Root cause (via Claude Code, reading `blueskyClient.js`): **NOT credentials and NOT naive login-per-call** — a concurrency race in `ensureSession()` with no single-flight guard; concurrent callers hitting a cold session each fired their own `createSession`, tripping Bluesky's rate limit -> ~2h cooldown that blocked all Bluesky incl. posting.
  - **Fix (commit `a15bbb7`):** single-flight guard so concurrent callers share one session establishment (refresh-first, createSession last-resort); `searchPosts` self-heals a stale token via **refresh** (not createSession). Concurrency-tested (20 concurrent cold searches -> 1 createSession; 20 concurrent 401s -> 0 createSession + 1 refresh).
  - **Deployed:** pulled to VPS (HEAD `a15bbb7`), rebuilt web/worker/scheduler images, no stale cooldown/session files in volume, brought stack up. **VERIFIED:** cooldown spam now **0** (was 749), scheduler cycling normally.
  - **PENDING PASSIVE CONFIRM:** watch `bsky.app/profile` for actual posts on the next publish cycle; if still none after a couple cycles, fall back to rotating `BLUESKY_APP_PASSWORD` (must be an app-password). Threads 500 = monitor only.
- **REBOOT:** Applied the pending kernel security update. Pre-flight confirmed all 4 containers `unless-stopped` + docker enabled-on-boot. `sudo reboot` -> stack **AUTO-RECOVERED clean** (all 4 containers up, app 200, reboot flag cleared; kernel now `6.8.0-134`, was `-124`). Note: the reboot wiped the (already-ineffective) `DOCKER-USER` iptables rules — moot, the Caddy approach supersedes.
- **STILL REMAINING (M6 final item, next session):** Caddy + localhost-bind + Full-strict TLS to close origin exposure (origin still directly reachable on 80/443 — low risk, IP hidden from DNS). Plan: rebind web to `127.0.0.1:8080` (localhost only); install **Caddy on the HOST** (systemd, reboot-persistent) on `0.0.0.0:80+443` -> `127.0.0.1:8080`, TLS via a **FRESH Cloudflare Origin Cert** (generate ONE cert; **verify cert/key modulus MATCH before wiring** — prior failures were mixing cert+key across generations); Cloudflare SSL Flexible -> Full(strict); verify site works via Cloudflare + direct-IP refused + survives reboot. **THEN:** review "Block AI training bots"; soak -> **decommission old host** (still dormant fallback). Then the anti-drift return to the build plan: event graph (4c gate+breaker on live graph, un-park comprehension homepage `c9862ef`), then distribution / Scoop.

**2026-07-XX (Session) — SOCIAL CHANNELS DIAGNOSED (Bluesky + Threads).** Site healthy; FB + Instagram posting normally (~8h cadence). Two channels were dark:
- **BLUESKY:** fix `a15bbb7` **IS deployed** (confirmed HEAD) and working (no new createSession bursts), BUT the IP was already deep in Bluesky's rate-limit penalty from days of pre-fix hammering — a direct createSession test returned **HTTP 429 RateLimitExceeded** (NOT 401 — so `BLUESKY_APP_PASSWORD` is **VALID**). App now quiet (0 cooldown lines in last 30m). **Conclusion:** no further action needed — credential valid, code fixed, waiting out the rate-limit timer (app internal cooldown was ~18000s + Bluesky server-side throttle). Should self-heal on the next publish cycle once both expire. **ACTION (passive):** check `bsky.app/profile/nmc192.bsky.social`; if still dark in ~24h with no cooldown activity, clear the app's internal `bluesky-cooldown` file in the volume to force a fresh attempt (only once Bluesky's own throttle has lifted).
- **THREADS:** dark ~4 days, not even attempting recently. Token IS present + well-formed (201 chars, `THAA…` prefix = valid Threads long-lived token shape). But every call (both `/{user-id}` and `/me`) returns **HTTP 500 with EMPTY body** — while an intentionally-invalid token returns a clean `400 code:190`, and `graph.facebook.com` + `graph.threads.net` are both reachable from the container (network/egress/firewall all fine). A structurally-valid token that consistently empty-500s = **token expired/invalidated server-side**. **Conclusion:** NOT code, NOT network — `THREADS_ACCESS_TOKEN` needs **REGENERATION** via the Meta Developer console (Threads app, `threads_basic` + `threads_content_publish` perms, fresh long-lived token via the OAuth flow; the old one is past refresh). **ACTION (next session):** regenerate token in Meta console, test it, update `backend/.env` on the VPS (token straight from Meta into `.env`, never through chat), restart. Consider a token-refresh helper so it doesn't silently expire again (~60-day lifetime).
- **STILL REMAINING (unchanged):** M6 Caddy + localhost-bind + Full-strict TLS (origin still directly reachable on 80/443 — the reboot wiped the ineffective `DOCKER-USER` rules; Caddy is the real fix). Then AI-bots review, soak, decommission old host, and the anti-drift return to the event-graph build.

**2026-07-05 (Session, cont.) — THREADS FIXED (token regenerated).**
- **Root cause confirmed:** expired long-lived token (structurally valid `THAA…` but every call empty-500'd; Meta long-lived tokens expire at 60 days and **can't be refreshed after expiry**).
- **Fix:** regenerated via Meta Developer console -> `scoopfeeds` app -> "Access the Threads API" use case -> Settings -> User Token Generator -> Generate Access Token for the `scoop.feeds` tester account (both `threads_basic` + `threads_content_publish` were "Ready for testing"). New long-lived token (~202 chars) written to `backend/.env` on the VPS via `read` (never through chat); containers recreated (`up -d`) to load env. **VERIFIED:** `graph.threads.net/v1.0/me` returned **HTTP 200 + username `scoop.feeds`** (was empty-500). Threads will post on the next publish cycle (~60min cadence). `.env.bak-threads` (held the old token) removed.
- **IMPORTANT FOLLOW-UP:** this token expires **~2026-09-03** (60 days). Set a ~50-day reminder to regenerate, OR (better) build an **auto-refresh helper** calling `graph.threads.net/refresh_access_token` before expiry so it doesn't silently die again. The same 60-day expiry risk applies to `FACEBOOK_PAGE_TOKEN` — worth auditing.
- **SOCIAL STATUS:** Facebook/Instagram/X posting normally. **Threads = fixed** (new token verified). **Bluesky = code fixed (`a15bbb7`) + credential valid**, draining a rate-limit timer, self-healing (passive check `bsky.app/profile/nmc192.bsky.social`).
- **STILL REMAINING (unchanged):** M6 Caddy + localhost-bind + Full-strict TLS (last infra item; origin still directly reachable on 80/443). Then AI-bots review, soak, decommission old host, and the anti-drift return to the event-graph build.

**2026-07-06 (Session) — M6 COMPLETE — origin hidden + strict TLS. THE MIGRATION IS DONE.**
- Generated a **FRESH Cloudflare Origin Certificate**; cert/key verified as a matched pair via `openssl rsa` modulus SHA256 (identical) + `openssl s_server` (accepted the pair). **NOTE:** the prior "cert mismatch" that blocked us across earlier sessions was a **FALSE NEGATIVE** from using `openssl pkey -pubout | md5` — the certs were fine all along; verify pairs with `openssl rsa -noout -modulus | sha256` or `s_server`, **never** the pkey/md5 method.
- Installed **Caddy on the HOST** (apt, systemd service = reboot-persistent). Caddyfile: serves `scoopfeeds.com`/`www`/`vps` on 443 with `tls <origin.pem> <origin.key>`, `reverse_proxy` to `127.0.0.1:8080`, `encode gzip`. Cert files `chown caddy:caddy` (key 600, cert 644).
- Rebound web container `0.0.0.0:80` -> `127.0.0.1:8080` (localhost only) via override; `docker-proxy` now binds only `127.0.0.1:8080`. Reloaded Caddy onto 80/443 (80 = 308 redirect to HTTPS).
- Switched Cloudflare SSL mode **Flexible -> Full (strict)**.
- **VERIFIED:** site **HTTP/2 200 + cf-ray** via Cloudflare; caddy local (proper SNI, `--cacert origin.pem`) = 200; **DIRECT origin access on :80 and :443 now REFUSED** (empty) — origin hidden, reachable only through Cloudflare. (The `ufw` CF-only rules on 80/443 now actually apply, since Caddy is a host process, not Docker.)
- **FINAL STATE:** off the LVE shared host, on the Hostinger KVM 2 VPS, Cloudflare DNS, email intact (Hostinger MX untouched), **strict TLS end-to-end, origin hidden, self-ingesting.** Old host still **DORMANT** as rollback (not yet decommissioned).

**REMAINING (small tail, non-blocking):** (1) reboot-recovery test to prove the full hardened stack auto-returns (Caddy systemd + override persist — expected clean, not yet tested); (2) review Cloudflare "Block AI training bots" default; (3) soak, then **decommission old host**; (4) social: Threads posting slow + Bluesky still dark (both flagged prior — Threads token refreshed but low cadence, Bluesky rate-limit/credential — revisit separately); (5) Threads/FB token 60-day expiry auto-refresh helper. **THEN:** the anti-drift return to the build — event graph (4c gate+breaker, un-park comprehension homepage `c9862ef`), then distribution / Scoop.

**2026-07-07 (Session) — TWO FIXES: Bluesky Phase 1 deployed + port 3000 origin-exposure closed.**
- **BLUESKY (Phase 1 of recovery):** Deployed commit `1253051` (`searchPosts` no longer initiates createSession — `ensureSession({createIfMissing:false})`; only the publisher creates sessions; ±15% cooldown jitter). Root cause was a **self-perpetuating loop:** the frequent sentiment `searchPosts` fired a createSession probe the instant each 6h cooldown expired -> 429 -> re-armed another 6h window (failCount hit **16** ≈ the days dark). Pulled to VPS (HEAD `1253051`), rebuilt web/worker/scheduler, `up -d`. **VERIFIED:** cooldown spam dropped **119/6h -> 0**. **NOTE:** this does NOT restore posting immediately — Bluesky's account 429 is still live (credential is **VALID**, confirmed 429 not 401, no rotation needed). Phase 1 = stop re-arming so the penalty can drain.
  - **PHASE 3 (pending, passive):** periodically re-run the direct createSession test; once it returns **non-429**, clear the stale cooldown file (`docker run --rm -v scoopfeeds_scoop_data:/data alpine rm -f /data/bluesky-cooldown.json`) and restart -> posting resumes (session then persists to `bluesky-session.json`). If the post-window probe ever returns **401**, that flips to credential -> rotate app-password.
- **PORT 3000 EXPOSURE CLOSED:** Discovered (by testing directly from an external IP) that after the M6 hardening the web container **STILL published `0.0.0.0:3000`** publicly — base compose maps `3000:3000`, and Docker **MERGES** port lists so the override's `127.0.0.1:8080` didn't remove it; `ufw` does NOT block it (Docker bypasses the host firewall). `curl http://<origin-ip>:3000/api/healthz` returned 200 from a non-Cloudflare IP. **FIX:** override web `ports` with the **`!override`** tag (NOT `!reset` — `!reset` **EMPTIED** the ports list entirely, which the config-render **gate caught before applying** would have taken the site down). Rendered merged config confirmed web publishes **ONLY `127.0.0.1:8080`**, then recreated only web. **VERIFIED:** external `:3000` now refuses (was 200); site still 200 + cf-ray via Cloudflare; app 200 on `127.0.0.1:8080`; Caddy serving. **Origin now genuinely hidden on ALL ports (80/443/3000).** override + `.bak3` are untracked local ops files (kept untracked).
- **DURABILITY NOTE:** the localhost-only binding depends on `docker-compose.override.yml` (untracked, uses `!override`). It persists on disk + survives reboot (compose reads it), BUT is **NOT in git** — if `/opt/scoopfeeds` is ever re-cloned, 3000 re-exposes. Consider **committing the override** (or its port-hardening) so the fix is durable.
- **SOCIAL STATUS:** FB/IG/X normal; Threads token refreshed (cadence was low — monitor); Bluesky Phase 1 done (awaiting 429 drain).

**2026-07-14 — BLUESKY RESOLVED (stale handle).** The passive checks above were watching the wrong door: the account was upgraded to the custom-domain handle **`@scoopfeeds.com`**, retiring `nmc192.bsky.social` — so the old profile URL looked dark while the account itself was fine. `BLUESKY_HANDLE` + a fresh app password were updated in `backend/.env` on the VPS; direct probe returned **200**. Profile checks now go to `bsky.app/profile/scoopfeeds.com` (the two passive-check references above are superseded by this entry). Lesson recorded in `docs/ops/social_credentials.md`: **a platform identity change (handle/username migration) is a publisher config change** — it must be treated like a token rotation (update env, recreate, probe), not assumed cosmetic.

...
| ~~`VIDEO_GRAIN_STRENGTH`~~ | **removed 2026-09-03** | — | — | **Gone. There is no film grain in any format.** The flag applied a static `noise=alls=14:allf=u` field to every slide of every automated short, both orientations. Removed on DrJ's ruling — it read as the paper texture already ruled out — and the flag was deleted rather than defaulted to `0`, because a dormant flag is a way back to a look that was decided against. Measured before/after on the 6-slide fixture, same binary: 9:16 went 26.4s / 15.9 MB → 17.4s / 0.47 MB, 16:9 went 30.6s / 20.1 MB → 20.4s / 0.47 MB, and the background's per-pixel stdev went 6.5 → 0. `videoContrast.test.js` fails if a `noise=` node returns to the graph. |
...
| `VIDEO_MUSIC_BED_ENABLED` | unset (**off**) | `1` | restart | Procedural score bed under the automated shorts, ported from the long-form engine's v4 bed (synthesis verbatim; arrangement generalized from six film chapters to the short's cold-open → build → turn → kicker shape). The turn card strips the music to 0.40 — the pull-back is what makes the argument land. Sidechain-ducked under narration, loudnorm to −14 LUFS, limiter last. **The level balance is set by the `VIDEO_MUSIC_BED_*` rows below, all added 2026-09-16 after the shipped mix measured the bed 13.5 dB ABOVE the voice with the duck never engaging.** Renders in <1s per video (pure ffmpeg synthesis, no assets, no licensing). A bed failure never costs a video: the caller logs and ships the unscored file. Dark until eyeballed live per house convention. |
| `VIDEO_MUSIC_BED_UNDER_DB` | `-12` | default | restart | How far under the **measured** narration loudness the bed is staged, in dB — the level you hear IN THE GAPS. Not a fixed gain: `scoreShort` measures both stems and derives the bed's gain per video, so the balance holds when `VIDEO_VOICE_ID` changes. **This is the flag that fixes "the music is too loud."** Measured at the default: bed 17.5 dB under the voice during speech, 4.0 dB lift in the gaps. Before this existed the bed sat **13.5 dB ABOVE** the voice, because `buildBed`'s `alimiter` auto-levels the bed to the ceiling and nothing compared the two stems. Range −40…0. |
| `VIDEO_MUSIC_BED_DUCK_THRESHOLD` | `0.02` | default | restart | Sidechain threshold, in **LINEAR AMPLITUDE** (ffmpeg's unit — `0.02` is −34 dBFS), against a key normalised to `VIDEO_MUSIC_BED_KEY_REF_LUFS`. The shipped value was `0.12` (−18.4 dBFS) while the narration key sat near −30 dBFS, so it was **11.9 dB below threshold and the compressor never opened**: measured median gain reduction 0.00 dB across 164 speech windows. Reading this number as if it were dB is the trap. Range 0.001…0.5. |
| `VIDEO_MUSIC_BED_KEY_REF_LUFS` | `-20` | default | restart | The loudness the sidechain **key** is normalised to before the threshold is applied. The key is a control signal only — this gain reaches no output — and it exists so one threshold is correct for every TTS voice instead of one. Range −40…−6. |
| `VIDEO_MUSIC_BED_DUCK_RATIO` | `2.5` | default | restart | Sidechain ratio. Unchanged from the original: the bed is meant to step back under speech, not disappear. With the threshold corrected this yields ~9.5 dB of reduction under speech. Range 1…20. |
| `VIDEO_MUSIC_BED_VOICE_LEVEL` | `1` (**on**) | default | restart | Levels the narration (gain → compressor → peak ceiling) before the mix. **Required by the bed fix, not cosmetic.** A −27.5 LUFS / −6.1 dBTP narration needs +13.5 dB to reach −14 LUFS, which loudnorm **cannot do linearly** — it silently falls back to the dynamic gain ride the 2026-08-30 fix exists to eliminate, and `linear=true` in the filter string does not reveal it. The old too-loud bed hid this by carrying the programme loudness itself. `0` restores the unlevelled narration and, with it, the dynamic fallback. This **does** make the read more even; that is the trade and it is deliberate. |
| `VIDEO_MUSIC_BED_VOICE_TARGET_LUFS` | `-18` | default | restart | Where the narration is gained to before compression. The **achieved** loudness is then re-measured and the bed staged against that, because the compressor and limiter pull the level back below the requested target (measured −19.74 for an −18 target). Range −30…−6. |
| `VIDEO_MUSIC_BED_VOICE_CEILING_DB` | `-6` | default | restart | Peak ceiling for the levelled narration, in dBFS. Sets the crest factor the mix hands loudnorm, which is what decides whether linear normalisation is possible. Range −20…−1. |
...
| `DIGEST_RECIPIENT_EMAIL` | unset (no digest) | set (DrJ) | restart | Recipient of the owner digests: the X-post digest (09:00 UTC) and the **shot-engine shorts digest** (07:56 UTC — yesterday's published shorts with metrics, missed bars and an inline 12-frame contact sheet each; no-op when nothing was published by the shot engine). Needs SMTP_*. Preview: `GET /scoop-ops/video/digest/preview?day=YYYY-MM-DD`; send: `POST /scoop-ops/video/digest/send-now`. |
| `VIDEO_WORD_CAPTIONS_ENABLED` | unset (**off**) | unset | restart | Word-by-word burned captions on the automated shorts (`videoWordCaptions.js`): chunks of up to three words at y 1190, the word being spoken in lime, Anton with a black stroke. Timed off the per-word alignment `videoVoice` already caches (#141) — a clip with no timings (cached before timestamps, or the plain-endpoint fallback) gets **no** captions on that slide rather than guessed ones. A chunk whose words are all in the card's own display type is suppressed. Drawn as satori PNG strips and overlaid, so it needs neither libass nor drawtext. A failure costs that slide its captions, never the video. Also on when `VIDEO_SHOT_ENGINE_ENABLED=1`. Gated on the literal string `"1"`. |
| `VIDEO_THUMBNAIL_ENABLED` | unset (**off**) | unset | restart | Custom poster frame for the automated shorts, 1080x1920, rendered through the **same satori + resvg path as the slides** so it cannot drift from the frames it fronts. Before this, every platform picked its own frame — YouTube at random, Instagram from `thumb_offset: 1000` (whatever is on screen at 1.0s, usually the title mid-animation). **The picture is NOT chosen here**: the thumbnail reuses the subject visual `produceVideo` already resolved, and never searches for one of its own — a second selection path is a second thing that can disagree with the spec's declared subject. No subject visual resolved → type on the house ground, which is a correct thumbnail, not a degraded one. **The hook is constrained to the centre 1080x1080** because Instagram centre-crops a Reel cover to a square on the profile grid, so a bottom-anchored hook is one nobody browsing ever reads. Wired per platform: YouTube `thumbnails.set` after upload (~50 quota units, against the same 10,000/day budget as the 1,600-unit upload); Instagram `cover_url` on the Reels container (replaces `thumb_offset`, which Meta ignores when a cover is given); Facebook `cover_url` at the `/video_reels` finish phase — **whether that surface accepts one is UNCONFIRMED** (developers.facebook.com is unreachable from the build network), so it is sent optimistically and the call retries without it on a parameter-specific rejection, which settles the question in the logs on the first published Reel. Rule 0 still gates the upload: the thumbnail is drawn from the same spec and subject visual the video was gated on, and is set only after that gate has passed. A thumbnail failure never costs a video — `renderThumbnail` returns null and the platforms keep their own frames. Gated on the literal string `"1"`. |

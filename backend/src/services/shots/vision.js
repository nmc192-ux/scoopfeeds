...
Answer ONLY JSON:
{"frames":[{"i":0,"shows_subject":true|false,"score":0-10,"sensitive":true|false,"banner":true|false,"note":"<=12 words"}],
...
Judge only what is visible. Do not guess.`;
  ...
  const picks = rows
    .filter((x) => x && x.shows_subject && !x.sensitive && Number.isInteger(x.i) && frames[x.i])
    .sort((a, b) => (b.score || 0) - (a.score || 0))
    .slice(0, 3)
    .map((x) => ({ t: frames[x.i].t, score: Number(x.score) || 0, note: String(x.note || "").slice(0, 80), banner: Boolean(x.banner) }));
...
Answer ONLY JSON:
{"matches":true|false,"sensitive":true|false,"screenshot":true|false,"private_person":true|false,"reason":"<=20 words"}
...
Judge only what is visible.`;
  ...
  const p = r.parsed || {};
  const usable = Boolean(p.matches) && !p.sensitive && !p.screenshot && !p.private_person;
  return { ok: true, usable, matches: Boolean(p.matches), sensitive: Boolean(p.sensitive), screenshot: Boolean(p.screenshot),

...
// ── 2.2.d COI ─────────────────────────────────────────────────────────────────
function coiEnv() {
  const { db, cleanup } = makeTestDb({ prefix: "scoring-coi-" });
...
function sevEnv(route) {
  const { db, cleanup } = makeTestDb({ prefix: "scoring-sev-" });
  ...
  for (let i = 0; i < 5; i++) ins.run(`a${i}`, `https://${DOMAIN}/news/${i}`, `Article ${i}`, "SevSrc", NOW - i * 86400000, NOW);

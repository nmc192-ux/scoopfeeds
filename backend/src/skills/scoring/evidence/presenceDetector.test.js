...
test("pre-pass — discovery runs once (scrape stale), skips when fresh; own-DB runs regardless; evidence-only", async () => {
  const { db, cleanup } = makeTestDb({ prefix: "scoring-pp-" });
  ...
  const insA = db.prepare(`INSERT INTO articles (id,url,title,category,source_name,published_at,fetched_at,is_duplicate) VALUES (?,?,?,'tech',?,?,?,0)`);
  for (let i = 0; i < 5; i++) insA.run(`a${i}`, `https://mocknews.example/news/${i}`, `Article ${i}`, "MockNews", NOW - i * 86400000, NOW);
